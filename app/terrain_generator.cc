...
[[nodiscard]] float AbsFloat(float value) noexcept {
    return value < 0.0f ? -value : value;
}

[[nodiscard]] float EvaluateLayerMask(const TerrainFeatureConfig &layer,
                                      float world_x,
                                      float world_z,
    ...
    const float mask = noise.Value(world_x - layer.offset_x_units * 0.37f + 8192.0f,
                                   world_z + layer.offset_z_units * 0.41f - 4096.0f,
                                   layer.mask_cell_size_units);
    return SmoothStep(layer.mask_threshold,
                      layer.mask_threshold + std::max(layer.mask_softness, 0.0001f),
                      mask);
}

[[nodiscard]] float EvaluateFeature(const TerrainFeatureConfig &layer,
                                    float world_x,
                                    float world_z,
                                    const TerrainNoise &noise) noexcept {
    const float sample = noise.Fractal(world_x + layer.offset_x_units,
                                       world_z + layer.offset_z_units,
                                       layer.noise_cell_size_units,
                                       layer.octaves);
    const float centered = sample * 2.0f - 1.0f;

    switch (layer.kind) {
        case TerrainFeatureKind::Plains:
            return centered * layer.amplitude_units * 0.5f;
        case TerrainFeatureKind::Hills:
            return centered * layer.amplitude_units;
        ...
        case TerrainFeatureKind::Detail:
            return centered * layer.amplitude_units;
    }
...
float TerrainNoise::Fractal(float world_x,
                            float world_z,
                            float cell_size_units,
                            int octaves) const noexcept {
...
void TerrainLayerStack::Clear() noexcept {
    layer_count_ = 0;
}

float TerrainLayerStack::Evaluate(float world_x,
                                  float world_z,
                                  const TerrainNoise &noise) const noexcept {
...
std::size_t TerrainLayerStack::Count() const noexcept {
    return layer_count_;
}
...
TerrainGenerator::TerrainGenerator() noexcept
    : TerrainGenerator(TerrainGeneratorConfig::MinecraftLike()) {}

TerrainGenerator::TerrainGenerator(TerrainGeneratorConfig config) noexcept
    : config_(config) {}

int TerrainGenerator::HeightAtVoxel(int voxel_world_x,
                                    int voxel_world_z,
                                    int voxels_per_world_unit,
    ...
    const int min_height = std::clamp(static_cast<int>(config_.min_height_units *
                                                       static_cast<float>(safe_voxels_per_unit) +
                                                       0.5f),
                                      0,
                                      max_height);
