...
namespace CoreEngine {
    ...
    void GeometryBatch::AddInstance(const RenderInstance &instance) {
        if (inline_instance_count_ < inline_instances_.size()) {
            inline_instances_[inline_instance_count_] = instance;
            ++inline_instance_count_;
            return;
    ...
    void GeometryBatch::ClearInstances() {
        inline_instance_count_ = 0;
        overflow_instances_.clear();
    }

    std::span<const RenderInstance> GeometryBatch::InlineInstances() const {
        return {inline_instances_.data(), inline_instance_count_};
    }

    std::span<const RenderInstance> GeometryBatch::OverflowInstances() const { return overflow_instances_; }

    std::size_t GeometryBatch::InstanceCount() const { return inline_instance_count_ + overflow_instances_.size(); }
    ...
    std::size_t GeometryBatchAccumulator::BatchKeyHash::operator()(const BatchKey &key) const noexcept {
        const auto mesh_id = static_cast<std::uint64_t>(key.mesh.id);
        const auto mesh_generation = static_cast<std::uint64_t>(key.mesh.generation);

        std::uint64_t hash = mesh_id;
        hash ^= mesh_generation + 0x9e3779b97f4a7c15ull + (hash << 6u) + (hash >> 2u);
        return static_cast<std::size_t>(hash);
    }

    void GeometryBatchAccumulator::Add(MeshHandle mesh, const Math::Mat4 &transform) {
        const BatchKey key{.mesh = mesh};
        const auto it = batch_indices_.find(key);
        ...
        const std::size_t index = active_batch_count_;
        if (index == batches_.size()) {
            batches_.push_back(GeometryBatch{});
        }

        GeometryBatch &batch = batches_[index];
        batch.mesh = mesh;
        batch.ClearInstances();
        batch.AddInstance(RenderInstance{transform});
    ...
    void GeometryBatchAccumulator::Reserve(std::size_t expected_instances) {
        if (expected_instances == 0) {
            return;
        }
    ...
    void GeometryBatchAccumulator::Clear() {
        for (std::size_t i = 0; i < active_batch_count_; ++i) {
            batches_[i].ClearInstances();
        }
