...
namespace CoreEngine {
    ...
    const char *ToString(NetworkDisconnectReason reason) noexcept {
        switch (reason) {
            case NetworkDisconnectReason::None:
                return "None";
            case NetworkDisconnectReason::LocalShutdown:
                return "LocalShutdown";
            case NetworkDisconnectReason::RemoteClosed:
                return "RemoteClosed";
            case NetworkDisconnectReason::TimedOut:
                return "TimedOut";
            case NetworkDisconnectReason::ProtocolMismatch:
                return "ProtocolMismatch";
            case NetworkDisconnectReason::BuildMismatch:
                return "BuildMismatch";
            case NetworkDisconnectReason::AuthenticationFailed:
                return "AuthenticationFailed";
            case NetworkDisconnectReason::LobbyFull:
                return "LobbyFull";
            case NetworkDisconnectReason::TransportError:
    ...
    void NetworkSession::BeginClientLobby(std::uint64_t lobby_id,
    ...
    void NetworkSession::SetState(NetworkSessionState state) noexcept {
        state_ = state;
    }
    ...
    NetworkPeer &NetworkSession::AddOrUpdatePeer(PeerId peer_id,
    ...
    void NetworkSession::RemovePeer(PeerId peer_id) noexcept {
        std::erase_if(peers_, [peer_id](const NetworkPeer &peer) {
            return peer.id == peer_id;
        });
    }

    NetworkPeer *NetworkSession::FindPeer(PeerId peer_id) noexcept {
        const auto it = std::ranges::find_if(peers_, [peer_id](const NetworkPeer &peer) {
            return peer.id == peer_id;
        });
